...
void RVND(Resultado& solucao, const vector<int>& d, const vector<vector<int>>& c, int Q) {
    ...
    while (!vizinhancas.empty()) {
        ...
        int indice = distribuicao(rng);       // sorteia um indice válido
        int k = vizinhancas[indice];          // pega o indice da vizinhança

        bool melhorou = false;

        switch (k) {

            case 1: melhorou = aplicarRelocate(solucao, d, c, Q);
            break;
            case 2: melhorou = aplicarSwapGeral(solucao, d, c, Q);
            break;
            case 3: melhorou = aplicarRelocateInter(solucao, d, c, Q); 
            break;
            case 4: melhorou = aplicarSwapInterRotasGeral(solucao, d, c, Q);
            break;
        }
...
bool aplicarSwapInterRotasGeral(Resultado &solucao, const vector<int> &d, const vector<vector<int>> &c, const int Q){
            ...
            for (int i = 1; i < rota1.caminho.size() - 1; i++){
                for (int j = 1; j < rota2.caminho.size() - 1; j++){
                    ...
                    // custo da troca (reconectando arestas nos dois lados)
                     ...
                     int novo_valr1 = (c[rota1.caminho[i - 1]][rota2.caminho[j]] + c[rota2.caminho[j]][rota1.caminho[i + 1]]) 
                                - (c[rota1.caminho[i - 1]][rota1.caminho[i]] + c[rota1.caminho[i]][rota1.caminho[i + 1]]); // efeito na rota1



                     int novo_valr2 = (c[rota2.caminho[j - 1]][rota1.caminho[i]] + c[rota1.caminho[i]][rota2.caminho[j + 1]]) 
                              - (c[rota2.caminho[j - 1]][rota2.caminho[j]] + c[rota2.caminho[j]][rota2.caminho[j + 1]]); // efeito na rota2

                     int novo_valor = novo_valr1 + novo_valr2;
...
bool aplicarSwapGeral(Resultado &solucao, const vector<int> &d, const vector<vector<int>> &c, const int Q){
...
bool aplicarRelocateInter(Resultado &solucao, const vector<int> &d, const vector<vector<int>> &c, const int Q){
...
bool aplicarRelocate(Resultado &solucao, const vector<int> &d, const vector<vector<int>> &c, const int Q){
    int melhor_r = -1;
    int melhor_i = -1;
    int melhorValor = 0;
